...
// https://zhuanlan.zhihu.com/p/446210109?utm_source=zhihu&utm_medium=social&utm_oi=32280114364416
...
int app_setup_uring(struct submitter *s) {
     ...
     * 不过，较旧的内核仅映射到提交队列中。
     * */
    sq_ptr = (unsigned *)mmap(
        0, sring_sz, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE, s->ring_fd, IORING_OFF_SQ_RING);
    ...
    if (p.features & IORING_FEAT_SINGLE_MMAP) {
        ...
        /* 分别映射到旧内核中的完成队列环形缓冲区 */
        cq_ptr = (unsigned *)mmap(
            0, cring_sz, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE, s->ring_fd, IORING_OFF_CQ_RING);
    ...
    /* 映射到提交队列条目数组 */
    s->sqes = (struct io_uring_sqe *)mmap(0, p.sq_entries * sizeof(struct io_uring_sqe), PROT_READ | PROT_WRITE,
